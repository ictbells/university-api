...
<head>
  ...
  <title>Receipt {{ $payment->receipt_no ?? $invoice->number }}</title>
  <style>
...
<body>
  <div class="sheet">
    ...
    <table>
      <tr><th>Category</th><td>{{ str_replace('_', ' ', ucfirst($invoice->category)) }}</td></tr>
      <tr><th>Payer</th><td>{{ $invoice->user?->name }}</td></tr>
      @if (!empty($applicant_ref))
        <tr><th>{{ $applicant_ref_label }}</th><td>{{ $applicant_ref }}</td></tr>
      @endif
      ...
      <tr><th>Amount paid</th><td class="total">₦{{ number_format((float) $payment->amount, 2) }}</td></tr>
    </table>
    ...
    <p class="footer">
      This receipt was generated electronically by {{ $university }}. Keep it for your records.<br>
