...
| `admissions.clear` | Clear admitted applicants after acceptance payment and physical verification |
...
| **PG research / purpose length** | Min/max words (0–5000) | Word limits for postgraduate Research interest (default max 150) and Statement of purpose (default max 500). Min 0 = no floor. Max 0 = no word cap (character limits still apply). Enforced on the applicant form and staff application file; not on applicant import. |
| **Letter signatories** | Registrar name/title/signature; PG name/title/signature | Registrar fields print on undergraduate and JUPEB admission letters (and official transcripts). Postgraduate fields print on postgraduate admission letters. Blank names use the letter defaults. |
...
Assigning programme fees (the Assign fees POST) and copying a schedule wait when **Create or Update** is on for **Fees & payments**. Generating a semester fee for all students waits when **Create** is on for **Fees & payments**.
...
- **Physical clearance** — Undergraduate, JUPEB, and Postgraduate queues (`admissions.clear`). Link each channel separately in office setup.
...
After the applicant pays acceptance, the file waits at `acceptance_paid`. They come to campus with original documents. Admissions staff clear them **individually** or in **bulk** on the matching **Applications → Physical clearance** channel page (or the Clear action on that channel's pipeline). Clearing records the visit and creates (or reattaches) the student record. Undergraduate, Direct Entry, transfer, and postgraduate students are issued a matric number immediately and **emailed that number** — they must then sign in with matric (application number and JAMB no longer work). JUPEB students get a student record without a matric until staff assign one on **Academic → JUPEB matric numbers** (that assignment is emailed separately). Already-matriculated files from before this step are treated as already cleared. To email matric numbers already issued before this mail existed: `php artisan students:email-matric` (sends immediately; preview with `--dry-run`; postgraduate only: `--entry-mode=pg`; JUPEB: `php artisan jupeb:email-matric`).
...
- **Application session** — Academic → Application Setup → Application sessions (intake). Open when **Accepting applications** is on and today is within the open and close dates. Application and acceptance fees are **not** edited here; they live in **Fees & payments → Fee items** as `application_fee` and `acceptance_fee` lines **per entry mode** (UTME, DE, JUPEB, Transfer, PG). Those catalog lines are reused when the category moves to a new academic session — do not create new fee items for the new year. Invoices use the catalog amount and fall back to a stored session amount only if no catalog line exists.
...
A NIN that already belongs to an applicant or student **cannot create a second account**. Signup tells them to sign in with matric number, application number, or JAMB (Forgot password if needed). An applicant also **cannot start a second form** (for example UTME after postgraduate) while any application is still open — withdrawn, rejected, or matriculated files do not block. Enrolled students cannot apply again until Registry confirms graduation; withdrawn and alumni students can start a **new application** on the same portal account. That later form is linked to the same login, so contact details stay shared. On the student portal, NIN biodata (names, date of birth, gender, NIN, NIN passport) stays locked. Staff can edit the **NIN** on the application file; after a NIMC correction they save the new number if needed, then staff with **Re-verify NIN** use **Resync from NIN** to refresh the locked identity fields. When a later offer’s acceptance fee is paid, the existing student record is reused — **one matric number per person**.

#### Candidate data

Upload JAMB candidate spreadsheets for the application session. Applicants can still create an account and submit without being on the list; after submit the file is marked `validated` or `pending` against that session’s list.
...
**Required columns (all categories):** email, first_name, last_name. UTME and Direct Entry also require `jamb_registration`. **NIN, phone, and first_choice_programme_id are optional.** If a NIN is present it must be 11 digits. If phone is filled it must be a valid Nigerian or international number. If programme id is filled it must already exist for that entry mode (copy from the Programmes lookup sheet **id** column). Leave phone and programme blank when the applicant will set them after sign-in (phone from NIN; programme on the form). Submit is still blocked until a programme is chosen.

**`old_application_number`** is optional. If filled, that value becomes this system’s application number (use it to match pending invoices). If blank, the system **generates** `APP/{year}/{#####}` (for example `APP/2026/00001`). A generated number will **not** match a custom invoice key such as `BELLS-APP-NUM-0001`. To post pending invoices, copy the **same** identifier used on the invoice sheet into `old_application_number` **or** use the same JAMB on both sheets.

**Documents are not imported** from Excel. Import **does not submit** the application: rows stay at **`form_in_progress`** so the applicant must update their records, re-upload required documents, and submit after they sign in. Incomplete rows also stay in the form so they can finish missing fields. The credentials email tells them they **must update their records before submitting**. Import invoices first when application fee was already paid (keyed by `application_number` or `jamb_registration`); this step posts matching rows, links them to the application, and records the fee as paid. Otherwise an **unpaid** application fee is generated from the fee catalog for that session’s entry mode (falling back to the session amount if no catalog line exists). After NIN verify, the locked **Phone from NIN** replaces any Excel phone; the imported number is kept as alternate if none was imported.

After import, the applicant signs in on the student portal with **application number or JAMB + password** (not email) and **verifies NIN** in the application form if it was not verified during upload. Duplicate email, NIN, JAMB, or application number is skipped.
...
| Role | Upload lane | Org scope | Senate / Release |
|------|-------------|---------|------------------|
| Exam Officer (`exam-officer`) | All | Campus | Yes |
| College Exam Officer (`faculty-exam-officer`) | College courses in their academic college | `staff.department_id` → college | No |
...
Registrar and Super Admin remain campus-wide. Committee of Deans approval of **general-lane** rows is not limited to the officer’s college. Create/update/import/submit/approve/Senate/release still go through the **HOD office inbox** (`officeGate`) when that office requires approval.
...
| Upload Score | Bulk draft import (`matric,score` or ca/exam columns). Registration is optional. Offerings are lane-filtered. | `results.import` |
| College | College grade list, submit drafts to Committee of Deans, print college list. **Grade** is the A–F letter from the total score and grading scale (staff do not type it). Print requires a semester. | `results.submit` |
| Committee of Deans | Approve/return college submissions (free-text return note); print committee list | `results.faculty_approve` |
| Senate | Senate-ready/cleared records list; Senate clear / request corrections; printable Senate list (college scope includes general-lane rows) | `results.board` |
...
Grade create/update/import/status changes are written to the platform **Audit** trail (`module = results`), not a separate Results audit screen. Result entry, College, Committee of Deans, Senate, and release lists accept **session**, **level**, **sitting**, **course**, and **matric** filters (term pickers stay where the list is term-based). College / Committee of Deans / Senate lists download as HTML, PDF, or Word.

**Typical flow**

1. Assign Results portal links under Department Setup (`results`, `results-students`, `results-department`, `results-approvals`, `results-board`, `results-release`, etc.) and grant the matching exam-officer role (or equivalent permissions). College, department, and GS officers should not receive Senate or Release links.
2. Review **Grading scale** before first use.
3. Enter or import drafts on Result entry / Upload Score (registration may follow later).
4. On **College**, review drafts and submit to the Committee of Deans; print the college list as needed.
5. On **Committee of Deans**, approve for Senate (or return with a note). Print the committee list as needed. HOD inbox may still apply.
6. On **Senate**, review the records list, then Senate clear (may require office approval).
...
- **Fees & payments** — **Payment dashboard** (university financial statement: fee collections, invoices issued, outstanding receivables, wallet liability; download PDF/Excel/Word; remittance breakdown can include **by fee item**), **Fee categories** (charge types, including programme-schedule vs operational) and **Fee items** (priced lines: **application and acceptance fees per entry mode**, **official transcript fees per programme and transcript type**, **Clinic services** visit charges, **Semester fee** (one university-wide amount for enrolled students each academic term — not a programme schedule line), and **school-fee installment shares**: any programme-schedule catalog line — tuition, ICT, laboratory, infrastructure, and the rest — can be tagged 1st/2nd/3rd/4th 25% or optional Full 100% pay-at-once; application, acceptance, and transcript fees stay online-only and cannot use shares), rebates, **Programme fees** (assign catalog lines to programmes with a naira override per line; select several levels at once when the amount is the same; copy a schedule to other programmes in the same college), **Generate invoice** (enter matric number for operational catalog items; also **Generate semester fee** for every active enrolled student for a selected term — see below), **Payments** (staff can **View receipt** on a paid invoice or successful wallet top-up; see **Official Receipt** below), **Students Financial Status** (same billed/paid/outstanding as the student portal **Financial status** page), **Import invoices**, and **Import wallet history** (`finance.invoices.manage`; dedicated import permission `finance.invoices.import` is also accepted). Invoices filter by session and level; Programme fees filter by level. When schedule lines are tagged with installment shares, student school-fee invoices bill those fixed amounts (already-paid fee items are skipped; 50% bills unpaid 1st + 2nd slices). Untagged schedules still pro-rate the full total by 25/50/75/100. The **Medical levy** is a programme schedule charge. **Clinic services** is an operational catalog: Finance sets name and amount; clinic staff attach those lines to a visit (quantity only) and cannot type prices. Hostel **Accommodation** on a school-fee sheet is a schedule catalog item, not a hostel-bed charge.
...
Use these when moving continuing students **or applicants** from another portal. **Do not** invent a “legacy registered” flag or auto-credit wallets from invoice `paid_amount`.

1. **Fees & payments → Import invoices.** Spreadsheet keyed by `matric_number`, `application_number`, or `jamb_registration` (at least one; the first filled is the **account key** shown on the page). Application fee is often paid with APP or JAMB before a matric exists. One row is one invoice. Extra rows with the same `invoice_number` add extra payments. Two (or more) rows for the same payer with **different** invoice numbers show as **N row(s)** under that key (for example two fees for `BELLS-APP-NUM-0001`). Category may be `application_fee`, `tuition`, `acceptance_fee`, `semester_fee`, and the rest of the catalogue. Tuition requires `installment_percent` (25/50/75/100). `paid_amount` records money received on the invoice; it does **not** credit the wallet.

   **Screen meaning:** **Posted** = attached to a student or applicant. **Pending** = imported but waiting for a matching account. **Skipped** = row errors. **Accounts waiting** = distinct pending keys. **Pending by account key** lists each key and how many invoice rows sit on it.

   If no matching student or applicant exists yet, rows stay **pending** until Import students or Import applicants runs. Matching is exact (case/spaces ignored): the applicant’s `old_application_number` (or generated `APP/{year}/{#####}`), JAMB, or the student’s matric must equal the invoice key. A custom invoice `application_number` such as `BELLS-APP-NUM-0001` posts only if that same string is imported as `old_application_number` (or the same JAMB is on both files). On **Import applicants**, matching pending rows are posted and linked; if none match, an unpaid application fee is generated from the fee catalog for that session’s entry mode (falling back to the session amount if no catalog line exists).
...
4. Leave **Verify NIN** off unless Prembly should run during the upload for rows that already have a NIN. Leave `nin` blank when the old portal did not store it; applicants verify after they sign in.
...
7. Tell applicants to sign in with application number or JAMB (not email) and the password they received (or the old plaintext password if it was supplied), then verify NIN in the application form.
...
| 1.36 | Aug 2026 | Platform team | Application settings set postgraduate research-interest and statement-of-purpose word limits |
| 1.37 | Aug 2026 | Platform team | Final application submit is blocked when that category’s application window is closed |
| 1.38 | Aug 2026 | Platform team | Fees & payments sidebar item Invoices is labelled Payments |
| 1.39 | Aug 2026 | Platform team | Applicant and student import do not require NIN; imported students stay enrolled and verify NIN after login |
| 1.39 | Aug 2026 | Platform team | Returning students sign in (not signup); NIN biodata is resynced by staff; one matric is reused on a later offer |
| 1.40 | Aug 2026 | Platform team | Students have a Financial status page matching staff; staff can view paid-invoice receipts with the same particulars breakdown |
