...
class ApplicantImportService
{
        ...
        if ($application->entry_mode === 'pg') {
            ...
            for ($i = 1; $i <= 3; $i++) {
                ...
                $referees[] = [
                    ...
                    'phone' => PhoneNumber::importValue($data["referee_{$i}_phone"] ?? null, 'referee phone'),
                ];
            }
    ...
    private function utmePayload(array $data): ?array
    {
        ...
        if (count($utmeSubjects) === 4 && filled($data['utme_aggregate'] ?? null)
            && ! ApplicationFormSteps::aggregateMatchesSubjectTotal([
                'aggregate' => $data['utme_aggregate'],
                'subjects' => $utmeSubjects,
            ])) {
            throw new RuntimeException('The four UTME subject scores must add up to the aggregate.');
        }
